    ...
    def __init__(
        ...
        *,
        ...
        if not self.force_irreps_out:
            self._output_irreps_unsimplified = self._output_irreps_unsimplified.filter(keep=irreps_in_regrouped)
        ...
        for ins in self._linear.instructions:
            ...
            if self.linear_type == "indexed":
                if self.num_indexed_weights is None:
                    raise ValueError("num_indexed_weights must be provided when 'linear_type' is 'indexed'")
            ...
            if self.linear_type in ["mixed", "mixed_per_channel"]:
                if self.weights_dim is None:
                    raise ValueError("weights_dim must be provided when 'linear_type' is 'mixed'")
                ...
                if self.linear_type == "mixed_per_channel":
                    if self.channel_in is None:
                        raise ValueError("channel_in must be provided when 'linear_type' is 'mixed_per_channel'")
    ...
    def __call__(self, weights_or_input, input_or_none=None) -> e3nn.IrrepsArray:
        ...
        if self.linear_type == "indexed":
            ...
            if self.weights_per_channel:
                raise NotImplementedError("weights_per_channel not implemented for indexed weights")

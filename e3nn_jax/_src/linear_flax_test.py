...
def test_linear_indexed(keys, irreps_in, irreps_out):
    linear = e3nn.flax.Linear(irreps_in=irreps_in, irreps_out=irreps_out, num_indexed_weights=10)
    x = e3nn.normal(irreps_in, next(keys), (128,))
    i = jnp.arange(128) % 10
...
def test_linear_mixed_per_channel(keys, irreps_in, irreps_out):
    linear = e3nn.flax.Linear(irreps_in=irreps_in, irreps_out=irreps_out, weights_per_channel=True)
    x = e3nn.normal(irreps_in, next(keys), (128,))

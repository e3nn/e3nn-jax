...
def reduced_tensor_product_basis(
    ...
    **irreps_dict,
    ...
    if perm_repr == _symmetric_perm_repr(len(irreps_tuple)):
        return reduced_symmetric_tensor_product_basis(irreps_tuple[0], len(irreps_tuple), epsilon=epsilon, keep_ir=keep_ir)
    ...
    *,
    ...
    for i, ir in enumerate(irreps):
        ...
        for n in range(1, order + 1):
            power = _reduced_symmetric_tensor_product_basis(ir, n, epsilon=epsilon)
    ...
    for term_powers in generate_tuples_with_fixed_sum(len(irreps), order):
        ...
        for next_term in non_zero_terms_reshaped[1:]:
            current_term, _ = reduce_basis_product(
                current_term, [0] * len(current_term.irreps), next_term, [0] * len(next_term.irreps)
            )
